"""
Interface Streamlit Otimizada para o Agente EDA
    ...
    with col1:
        if st.button("🚀 Perguntar", type="primary"):
            if user_input and st.session_state.agent:
                ...
                st.error("❌ Configure a API Key e carregue um arquivo primeiro!")
...
def main():
    ...
    with st.sidebar:
        ...
        if api_key:
            ...
            if st.session_state.agent is None:
                try:
                    st.session_state.agent = create_smart_agent(api_key)
        ...
        if st.button("📊 Usar Dataset de Exemplo"):
            sample_path = "/home/ubuntu/data/creditcard.csv"
            if os.path.exists(sample_path) and st.session_state.agent:
                try:
                    start_time = time.time()
                    
                    with st.spinner("⚡ Carregando e analisando..."):
                        result = st.session_state.agent.load_and_analyze_csv(sample_path)
    ...
    if st.session_state.file_loaded and st.session_state.dashboard_data:
        ...
        # Tela inicial
        st.info("👆 Configure sua API Key e carregue um arquivo CSV para análise instantânea!")
        ...
        with col1:
            ...
            **⚡ Análise Automática (Instantânea):**
            ...
            - Gráficos automáticos (histogramas, boxplots, heatmaps)
