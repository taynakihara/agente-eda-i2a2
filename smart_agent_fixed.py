"""
Agente Inteligente Otimizado - Versão Robusta
...
try:
    ...
    HAS_LANGCHAIN = False
    print("⚠️ LangChain não disponível - funcionalidade de chat limitada")

from auto_eda_fixed import AutoEDA
...
class SmartDataAgent:
    ...
    def answer_question(self, question: str) -> Dict[str, Any]:
        ...
        if self.llm:
            ...
            return {
                'response': 'Esta pergunta requer análise específica, mas o LLM não está disponível. Configure sua OpenAI API Key para perguntas mais complexas.',
                'needs_computation': False,
                'chart_paths': [],
                'data_source': 'no_llm'
            }
    ...
    def _check_precomputed_answers(self, question: str) -> Optional[Dict[str, Any]]:
        ...
        if any(keyword in question_lower for keyword in ['estatísticas', 'descritivas', 'resumo', 'summary']):
            stats = self.eda_results['descriptive_stats']
            
            if 'message' in stats:
                return {'answer': stats['message'], 'charts': []}
            ...
            main_cols = list(numeric_summary.keys())[:3]
    ...
    def _answer_with_llm(self, question: str) -> Dict[str, Any]:
        ...
        if not self.llm:
            return {
                'response': 'LLM não disponível. Configure sua OpenAI API Key para perguntas específicas.',
                'needs_computation': False,
                'chart_paths': []
            }
...
def create_smart_agent(openai_api_key: str = None) -> SmartDataAgent:
